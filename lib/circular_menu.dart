...
class _CircularMovementAnimation extends StatefulWidget {
  const _CircularMovementAnimation({required this.ring, required this.items});
  final Widget ring;
  final List<Transform> items;

  @override
  __CircularMovementAnimationState createState() => __CircularMovementAnimationState();
}

class __CircularMovementAnimationState extends State<_CircularMovementAnimation>
  ...
  void _runAnimation(Offset pixelsPerSecond, Size size) {
    ...
    print("$_prevRotation, $_rotation");
    if (_rotation > 360.0) _rotation -= 360.0;
    _rotation = Offset(widget.items[0].transform.getTranslation().x,
        widget.items[0].transform.getTranslation().y).distance;
    print("Rotation of Widget: $_rotation");
    _rotateAnimation = _controller.drive(
      Tween<double>(begin: _rotation-0.10, end: _rotation),
    );
    ...
    print("Units Per Second For X: $unitsPerSecondX, for Y: $unitsPerSecondY");
  ...
  @override
  void initState() {
    print("Movement Animation Initialised");
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onPanUpdate: (details) {
        double rotationDistance = (details.delta.dx / (size.width/5)) +
            (-details.delta.dy / (size.height/5));

        setState(() {
          _rotation += rotationDistance;
          print("Rotation Distance: $_rotation\t dividibility by 360: ${_rotation%360.0}");
        });
      },
      onPanEnd: (details) {
...
class FabCircularMenuState extends State<FabCircularMenu>
  ...
  void initState() {
    ...
    _rotateAnimation.addStatusListener(_updateStatus);
      ...
      child: Stack(
        ...
        children: <Widget>[
          ...
          GestureDetector(
            ...
            //       _circleValue = details.primaryDelta!;
            //     });
            //   }
            // },
            child: Transform(
              ...
              child: OverflowBox(
                ...
                child: Container(
                  ...
                  child: _CircularMovementAnimation(
  ...
  Transform _applyTransformations(Widget child, int index) {
    double angleFix = 0.0;
    ...
    final angle =
    vector.radians(360.0 / (widget.children.length - 1) * index + angleFix);
